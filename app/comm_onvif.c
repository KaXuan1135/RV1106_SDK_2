...
static char g_mainMulti_ip[24] = "224.224.0.123";
static int  g_mainMulti_port = 9966;

static char g_subMulti_ip[24] = "224.224.1.123";
static int  g_subMulti_port = 9966;

static char g_audioMulti_ip[24] = "224.224.2.123";
static int  g_audioMulti_port = 9966;
...
int app_get_capture_data(int ch,char *pbuff,int buff_size,int *ret_size)
{
	...
	os_dbg("app_get_capture_data.............\n");
	*ret_size = buff_size;
	if(app_capture_pic(ch*2,pbuff,  ret_size) < 0)
	{
		*ret_size = 0;
	}	
	return OS_SUCCESS;
...
{
		int width_height[MAX_RES_NUM][2] = {
			{176,144},
			{352,288},
			{704,288},
			{704,576}, //MD1
			...
			{960,540},
			{480,720},
			{2880,1616}, //RES_2880x1616
			{2560,1440}, //RES_4M
   		 };

	if (resolution > MAX_RES_NUM)
	{
...
static int g_resCap_P[MAX_RES_NUM][2] =
{
	{176,144},
	{352,288},
	{704,288},
...
int app_get_multicast_config(const char *token,ttmulticast_t *multicast)
{
	if(0 == strcmp(token,g_videoencoderconfiguration_m.configurationtoken))
	{
		strcpy(multicast->address.ipv4address, g_mainMulti_ip);
		snprintf(multicast->port, 8, "%d", g_mainMulti_port);	
	}
	else if(0 == strcmp(token,g_videoencoderconfiguration_s.configurationtoken))
	{
		strcpy(multicast->address.ipv4address, g_subMulti_ip);
		snprintf(multicast->port, 8, "%d", g_subMulti_port);	
	
	}
	else if(0 == strcmp(token,g_audioencoderconfiguration.configurationtoken))
	{
		strcpy(multicast->address.ipv4address, g_audioMulti_ip);
		snprintf(multicast->port, 8, "%d", g_audioMulti_port);	
		
	}
    strcpy(multicast->address.type, "IPv4");
	strcpy(multicast->ttl, "4");
	strcpy(multicast->autostart, "true");
	os_dbg("token:%s multiaddr: %s:%s ",token,multicast->address.ipv4address,multicast->port);
	return OS_SUCCESS;
}
	...
	if(0 == strcmp(pVideoencoderconfiguration->name,"VideoEncoder_1")) //master stream
	{
		if((app_check_Resolution(0,0,width,heigh)== OS_SUCCESS) &&
			...
			/*&&(quality > 21 && quality < 40) */)
			{
				...
				comm_param_setVideoEncodeParam(0, video_encoder);
	...
	else if(0 == strcmp(pVideoencoderconfiguration->name,"VideoEncoder_2")) // sub stream
	{
		if((app_check_Resolution(0,1,width,heigh)== OS_SUCCESS) &&
			...
			/*&&(quality > 21 && quality < 40)*/)
			{
				...
				video_encoder.Iframe_interval = encodinginterval;
				video_encoder.resolution = app_getResolution(width,heigh);
				comm_param_setSlaveEncodeParam(0, video_encoder);
...
int app_set_multicast_config(const char *token,ttmulticast_t * pMulticast)
{
	if(0 == strcmp(token,g_videoencoderconfiguration_m.configurationtoken))
	{
		g_mainMulti_port = atoi(pMulticast->port);
		memset(g_mainMulti_ip,0,sizeof(g_mainMulti_ip));
		strcpy(g_mainMulti_ip,pMulticast->address.ipv4address );
	}
	else if(0 == strcmp(token,g_videoencoderconfiguration_s.configurationtoken))
	{
		g_subMulti_port = atoi(pMulticast->port);
		memset(g_subMulti_ip,0,sizeof(g_subMulti_ip));
		strcpy(g_subMulti_ip,pMulticast->address.ipv4address );
	
	}
	else if(0 == strcmp(token,g_audioencoderconfiguration.configurationtoken))
	{
		g_audioMulti_port = atoi(pMulticast->port);
		memset(g_audioMulti_ip,0,sizeof(g_audioMulti_ip));
		strcpy(g_audioMulti_ip,pMulticast->address.ipv4address );
	}
	os_dbg("token:%s multicast : %s :%d",token,pMulticast->address.ipv4address,atoi(pMulticast->port));
	return OS_SUCCESS;
}
int app_start_multicast_stream(int ch,int ch_type)
{
	...
	if(ch_type == 0)
	{
		iRet= comm_start_multiRtp(g_mainMulti_ip,g_mainMulti_port,ch,ch_type);
	}
	else
	{
		iRet= comm_start_multiRtp(g_mainMulti_ip,g_mainMulti_port,ch,ch_type);
...
{
	app_init_uuid();
//	onvif_set_process_debug_level(5);
	onvif_init(NULL);
	return OS_SUCCESS;
}
